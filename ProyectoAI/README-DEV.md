...
This workspace contains 4 repositories for the Solveria platform:
...
### 4. `market-service/` (AlimentoZero Market Service)
- **Purpose:** Hybrid rescue-food marketplace for donations, discounted sales, reservations and pickup flows
- **Tech:** Spring Boot 3.4.0, Java 21
- **Port:** 8092 (default)
- **Architecture:** Clean Architecture (domain/application/infrastructure/api/bootstrap)
...
### Build Everything
```powershell
# Build all repos in correct order (core → iam → ai → market)
...
cd market-service
.\mvnw.cmd clean verify
.\mvnw.cmd spring-boot:run -pl modules/market-bootstrap
```
...
- **Market Service:** `market-service/README.md` (marketplace scope and API)
...
- [ ] All 4 repos build successfully: `.\scripts\build-all.ps1`
